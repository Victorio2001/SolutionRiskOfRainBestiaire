...
using RiskOfRain.Web.ViewsModels.Account;
using RiskOfRain.DataSource;
using RiskOfRain.Sender.Interfaces;
using RiskOfRain.Web.Controllers;
...
namespace RiskOfRain.Web.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(ILogger<HomeController> logger, IMailSender mailSender, 
        ...
        public async Task<IActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = new User { UserName = model.Email, Email = model.Email };
                var result = await _userDataSource.CreateAsync(user);
                if (result.Succeeded)
                {
                    result = await _userDataSource.AddLoginAsync(user, info);
                    if (result.Succeeded)
                    {
                        await _signInDataSource.SignInAsync(user, isPersistent: false);
        ...
        public async Task<IActionResult> SendCode(SendCodeViewModel model)
        {
            ...
            {
                await _mailSender.SendEmailAsync(await _userDataSource.GetEmailAsync(user), "Security Code", message);
            }
